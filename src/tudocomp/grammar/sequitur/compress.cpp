...
extern "C" {
#include <tudocomp/grammar/sequitur/arith.h>
#include <tudocomp/grammar/sequitur/stats.h>
#include <tudocomp/grammar/sequitur/bitio.h>
}
...
#ifndef SEQUITUR_VARS
#define SEQUITUR_VARS
...
using tdc::grammar::sequitur::rules;
using tdc::grammar::sequitur::symbols;
...
void uncompress()
{
  ...
  while (1) {
    ...
    else if (i == NOT_KNOWN)
    {
      ...
      std::cout << char(CODE_TO_TERM(j));
    }
    // symbol is a (known) terminal
    else if (IS_TERMINAL(i))
    {
      if (numbers) std::cout <<  int(CODE_TO_TERM(i)) << std::endl;
      else         std::cout << char(CODE_TO_TERM(i));
    }

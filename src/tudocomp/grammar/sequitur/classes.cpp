/****************************************************************************
 ...
 Notes:
  ...
  Modifications copyright (C) 2021 Etienne Palanga
 ****************************************************************************/
...
using namespace tdc::grammar::sequitur;
...
symbols *tdc::grammar::sequitur::rules::first() { return guard->next(); }
// pointer to  last symbol of rule's right hand
symbols *tdc::grammar::sequitur::rules::last() { return guard->prev(); }
...
int lookups    = 0;
int collisions = 0;
// int tdc::grammar::sequitur::occupied = 0;
symbols **table = 0;
...
// ***************************************************************************
symbols **tdc::grammar::sequitur::find_digram(symbols *s) {
    if (!table) {
        ...
        if (!quiet) {
            cerr << "Using " << memory_to_use / 1000000 << " MB of memory for the hash table." << endl;
...
void rules::reproduce() {
    ...
    for (symbols *p = first(); !p->is_guard(); p = p->next()) p->reproduce();
}
...
// **************************************************************************
ostream &tdc::grammar::sequitur::operator<<(ostream &o, symbols &s) {
    // extern int numbers;

    if (s.non_terminal())
        o << s.rule()->index();
    else if (numbers & do_uncompress)
        o << s.value() << endl;
    else if (numbers)
        o << '[' << s.value() << ']';
        ...
        o << '_';
    else if (s.value() == '\\' || s.value() == '(' || s.value() == ')' || s.value() == '_' || isdigit(s.value()))
        o << '\\' << char(s.value());
    else
        o << char(s.value());
...
void rules::output() {
    ...
    cout << "R" << number << " -> ";
    for (s = first(); !s->is_guard(); s = s->next()) cout << *s << ' ';
...
size_t rules::number_rules_recursive() {
    symbols *s;
    ...
    for (s = first(); !s->is_guard(); s = s->next())
        if (s->non_terminal() && s->rule()->index() == 0)
            max = std::max(s->rule()->number_rules_recursive(), max);
    return max;
}

...
namespace huff {
	...
	len_t* count_alphabet(const T& input) {
		typedef std::make_unsigned<std::string::value_type>::type value_type;
	...
	uint8_t* gen_codelengths(const len_t*const C, const uliteral_t*const map_from_effective, const size_t alphabet_size) {
		...
		assert_permutation_range<size_t*>(A,alphabet_size,alphabet_size,2*alphabet_size);
	...
	void huffman_encode(
		...
		DCHECK_GT(input_length, 0);
			...
			uint8_t map_to_effective[max_literal] {0};
			for(size_t i = 0; i < alphabet_size; ++i) {
	...
	void huffman_decode(
			...
			while(true) {
				...
				output << ordered_map_from_effective[accum_length[length]+ (value - firstcodes[length]) ];
	...
	void encode(tdc::io::Input& input, tdc::io::Output& output) {
		...
		std::stringstream str; // TODO: this is really BAD
		str << iview;
		huffman_encode(str, bit_os, str.str().length(), table.ordered_map_from_effective, table.ordered_codelengths, table.alphabet_size, table.codewords);
	}

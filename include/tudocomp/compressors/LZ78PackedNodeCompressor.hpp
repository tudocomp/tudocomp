...
class LZ78PackedNodeCompressor: public Compressor {
private:
...
public:
    inline LZ78PackedNodeCompressor(Config&& cfg):
        Compressor(std::move(cfg)),
        m_dict_max_size(this->config().param("dict_size").as_uint())
    {}

    inline static Meta meta() {
        Meta m(Compressor::type_desc(), "lz78packednode",
    ...
    virtual void compress(Input& input, Output& out) override {
        ...
        node_t jump_ancestor = node; //! ancestor whose string depth is a multiple of eight, needed for jumping
		...
		using cache_type = std::unordered_map<std::pair<uint64_t, lz78::factorid_t>, lz78::factorid_t, pair_hash>;
		cache_type cache;
		...
		while(is.good()) {
			...
			//	&& false
				...
				)
				{
					...
					const auto key = std::make_pair(character_buffer, node.id());
					...
					if(it != cache.end()) {
						...
						// std::cout << "OK node id " << it->first.second << " packed: " << it->first.first << " -> " << jumped_child.id() << std::endl;
						// }
						...
						node = jump_ancestor = jumped_child;
			...
			for(uint_fast8_t buffer_offset = 0; buffer_offset != read_chars; ++buffer_offset) {
				...
				if(child.is_new()) {
					...
					//if(string_depth % 8 == 0) { 
...
#ifndef NDEBUG//checks whether the move is valid
						{
							node_t checknode = jump_ancestor;
						...
						const auto key = std::make_pair(character_ringbuffer, jump_ancestor.id());
						// if(string_depth > 8) {
						// std::cout << "d: " << string_depth <<  ", [" << character_ringbuffer << "," << jump_ancestor.id() << "] -> " << child.id() << std::endl;
						// }
						DCHECK(cache.find(key) == cache.end());
						cache[key] = child.id();
					}
					jump_ancestor = node; //! reset 
					string_depth = 1;

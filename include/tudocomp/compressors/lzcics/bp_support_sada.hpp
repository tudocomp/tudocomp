...
*/
/*! \file bp_support_sadaM.hpp
    \brief bp_support_sadaM.hpp contains an implementation of a balanced
...
class bp_support_sadaM
{
    ...
    private:
        static_assert(0 < t_sml_blk, "bp_support_sadaM: t_sml_blk should be greater than 0!");
...
//#define USE_CACHE
        ...
        void copy(const bp_support_sadaM& bp_support)
        {
    ...
    public:
        ...
        //! Constructor
        explicit bp_support_sadaM(const bit_vector* bp): m_bp(bp),
            ...
            m_med_inner_blocks(0)
        {
            ...
            m_sml_block_min_max = sdsl::int_vector<>(2*m_sml_blocks, 0, bits::hi(t_sml_blk+2)+1);
            m_med_block_min_max = sdsl::int_vector<>(2*(m_med_blocks+m_med_inner_blocks), 0, bits::hi(2*m_size+2)+1);
            ...
            bit_vector root_children(m_size);
            size_type rc = 1;
        ...
        //! Copy constructor
        bp_support_sadaM(const bp_support_sadaM& bp_support)
        {
            copy(bp_support);
        }

        //! Move constructor
        bp_support_sadaM(bp_support_sadaM&& bp_support)
        {
            *this = std::move(bp_support);
        }

        //! Move Assignment operator
        bp_support_sadaM& operator=(bp_support_sadaM&& bp_support)
        {
            if (this != &bp_support) {
         ...
         */
        void swap(bp_support_sadaM& bp_support)
        {
            if(this != &bp_support) {
        ...
        //! Assignment operator
        bp_support_sadaM& operator=(const bp_support_sadaM& bp_support)
        {
            if (this != &bp_support) {
                copy(bp_support);
            }
        ...
        /*
                bp_support_sadaM& operator=(bp_support_sadaM&& bps)
                {
                    this->swap(bps);
                    return *this;
                }
...
/*            if ((*m_bp)[i]) {// if there is a opening parenthesis at index i return i
        ...
        */
        ...
        //! Serializes the bp_support_sadaM to a stream.
        /*!
        ...
        //! Load the bp_support_sadaM for a bit_vector v.
        /*!
        ...
        //direct BP string access for testing purposes
        const bit_vector& bp_string() const {
            return *m_bp;
        }
};

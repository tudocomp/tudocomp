...
struct SuffixTree {
	...
	const root_childrank_support root_childrank;
    ...
    cst_t::size_type root_child_rank(const cst_t::node_type& node) const {
        return root_childrank.rank(node);
    }
	 ...
	 * Returns the length of the label read from the edges on the path from the root to node
	 */
	cst_t::size_type str_depth_fast(const cst_t::node_type& node)const{

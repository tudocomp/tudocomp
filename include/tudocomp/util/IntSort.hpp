...
inline std::pair<std::vector<T>, bool> intsort_impl(const Iter begin, const Iter end, KeyExtract key_extract,
    ...
    int max_threads = 1;
#ifdef ENABLE_OPENMP
    max_threads = std::min<int>(omp_get_max_threads(), idiv_ceil(n, 1 << 17));
#endif
...
#pragma omp barrier
    ...
    inline void
    intsort(std::vector<T> &input, KeyExtract key_extract, const Key max_key = std::numeric_limits<Key>::max()) {

...
TEST(rank, rank1_8bit) {
    ...
    // interval [0,m]
    ASSERT_EQ(1U, rank1(v2, 0));
    ASSERT_EQ(2U, rank1(v2, 1));
    ASSERT_EQ(2U, rank1(v2, 7));

    ASSERT_EQ(0U, rank1(v60, 1));
    ASSERT_EQ(2U, rank1(v60, 3));
    ASSERT_EQ(4U, rank1(v60, 5));
    ASSERT_EQ(4U, rank1(v60, 7));

    for(uint8_t i = 0; i < 8; i++) ASSERT_EQ(i+1, rank1(v255, i));

    // interval [l,m]
    ASSERT_EQ(1U, rank1(v2, 0, 0));
    ASSERT_EQ(2U, rank1(v2, 0, 1));
    ASSERT_EQ(2U, rank1(v2, 0, 7));
    ASSERT_EQ(0U, rank1(v2, 2, 7));

    ASSERT_EQ(0U, rank1(v60, 0, 1));
    ASSERT_EQ(2U, rank1(v60, 0, 3));
    ASSERT_EQ(4U, rank1(v60, 2, 5));
    ASSERT_EQ(2U, rank1(v60, 4, 7));
    ASSERT_EQ(0U, rank1(v60, 6, 7));
    ASSERT_EQ(4U, rank1(v60, 0, 7));

    for(uint8_t i = 0; i < 8; i++) ASSERT_EQ(1U, rank1(v255, i, i));
}

TEST(rank, rank0_8bit) {
    // full vectors
    uint8_t v0   = 0b0000'0000; ASSERT_EQ(8U, rank0(v0));
    uint8_t v2   = 0b0000'0011; ASSERT_EQ(6U, rank0(v2));
    uint8_t v60  = 0b0011'1100; ASSERT_EQ(4U, rank0(v60));
    uint8_t v255 = 0b1111'1111; ASSERT_EQ(0U, rank0(v255));

    // interval [0,m]

    ASSERT_EQ(0U, rank0(v2, 0));
    ASSERT_EQ(0U, rank0(v2, 1));
    ASSERT_EQ(6U, rank0(v2, 7));

    ASSERT_EQ(2U, rank0(v60, 1));
    ASSERT_EQ(2U, rank0(v60, 3));
    ASSERT_EQ(2U, rank0(v60, 5));
    ASSERT_EQ(4U, rank0(v60, 7));

    for(uint8_t i = 0; i < 8; i++) ASSERT_EQ(i+1, rank0(v0, i));

    // interval [l,m]
    ASSERT_EQ(0U, rank0(v2, 0, 0));
    ASSERT_EQ(0U, rank0(v2, 0, 1));
    ASSERT_EQ(6U, rank0(v2, 0, 7));
    ASSERT_EQ(6U, rank0(v2, 2, 7));

    ASSERT_EQ(2U, rank0(v60, 0, 1));
    ASSERT_EQ(2U, rank0(v60, 0, 3));
    ASSERT_EQ(0U, rank0(v60, 2, 5));
    ASSERT_EQ(2U, rank0(v60, 4, 7));
    ASSERT_EQ(2U, rank0(v60, 6, 7));
    ASSERT_EQ(4U, rank0(v60, 0, 7));

    for(uint8_t i = 0; i < 8; i++) ASSERT_EQ(1U, rank0(v0, i, i));
}
...
TEST(rank, uint_16_32_64) {
    ...
    ASSERT_EQ(2U, rank1(v16));
    ASSERT_EQ(4U, rank1(v32));
    ASSERT_EQ(8U, rank1(v64));
    ...
    // interval [l, m]
    for(uint8_t i = 1; i <= 2; i++) {
        ASSERT_EQ(1U, rank1(v16, 8*(i-1), 8*i-1));
        ASSERT_EQ(1U, rank0(uint16_t(~v16), 8*(i-1), 8*i-1));
    }
    for(uint8_t i = 1; i <= 4; i++) {
        ASSERT_EQ(1U, rank1(v32, 8*(i-1), 8*i-1));
        ASSERT_EQ(1U, rank0(uint32_t(~v32), 8*(i-1), 8*i-1));
    }
    for(uint8_t i = 1; i <= 8; i++) {
        ASSERT_EQ(1U, rank1(v64, 8*(i-1), 8*i-1));
        ASSERT_EQ(1U, rank0(uint64_t(~v64), 8*(i-1), 8*i-1));
    }
}
...
TEST(select, basic) {
    ...
    for(size_t i = 1; i <= 8; i++) {
        ASSERT_EQ(8U*(i-1), select1(v64, i));
        ASSERT_EQ(8U*(i-1), select0(uint64_t(~v64), i));
    }
}
...
TEST(rank, bv) {
    NK_test([](size_t N, size_t K){
        ...
        for(size_t i = 1; i <= N/K; i++) ASSERT_EQ(1U, rank(K*(i-1), K*i-1));
